...
Class RoadThemes {
	...
	*/
	function __construct() {
		...
		add_filter( 'woocommerce_shortcode_products_query', array($this, 'road_woocommerce_shortcode_count'));
	...
	*/
	...
	function road_setup() {
		...
		set_post_thumbnail_size( 820, 9999 ); // Unlimited height, soft crop
		add_image_size( 'sozo-category-thumb', 750, 510, true ); // (cropped)
		add_image_size( 'sozo-category-full', 1144, 510, true ); // (cropped)
		add_image_size( 'sozo-post-thumb', 650, 420, true ); // (cropped)
		add_image_size( 'sozo-post-thumbwide', 570, 310, true ); // (cropped)
	}
	...
	*/
	...
	static function road_breadcrumb() {
		...
		if (!is_home()) {
			...
			elseif (is_day()) {echo "<span>".esc_html__('Archive for','sozo'); the_time('F jS, Y'); echo'</span>';}
			elseif (is_month()) {echo "<span>".esc_html__('Archive for','sozo'); the_time('F, Y'); echo'</span>';}
			elseif (is_year()) {echo "<span>".esc_html__('Archive for','sozo'); the_time('Y'); echo'</span>';}
			elseif (is_author()) {echo "<span>".esc_html__('Archive for','sozo'); echo'</span>';}
			elseif (isset($_GET['paged']) && !empty($_GET['paged'])) {echo "<span>".esc_html__('Blog Archives','sozo'); echo'</span>';}
			elseif (is_search()) {echo "<span>".esc_html__('Search Results','sozo'); echo'</span>';}
